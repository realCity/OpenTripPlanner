...
public class StreetUtils {
    ...
    private static int processIslands(
        ...
        boolean markIsolated, int maxIslandSize, int islandWithStopMaxSize, DataImportIssueStore issueStore) {
        ...
        for (Subgraph island : islands) {
            ...
            if(island.stopSize() > 0){
                ...
                if (island.streetSize() < islandWithStopMaxSize) {
                    depedestrianizeOrRemove(graph, island, issueStore, isolated, stats, markIsolated);
                    hadRemoved = true;
                    count++;
                }
            }else{
